...
which has been tackled by many researchers [10].

The algorithms which are currently used most frequently by available software are:
grid placement [15], tree placement (in cases where it is possible), and force-directed placement [14].
...
Regarding connection algorithms, most software today either use techniques such as shortest path connection [11]
, orthogonal edge routing [3] or various routing mechanics built around heuristics. Each approach tries to find a 
...
of finding the proper representation so that the number of intersections is 0 is NP [16], therefore there is no
