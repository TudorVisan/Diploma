...
which has been tackled by many researchers. The purpose of proper node placement is
to obtain a clear and easy to follow representation of a graph.

The algorithms which are currently used most frequently by available software are:
grid placement, tree placement (in cases where it is possible), and force-directed placement.

Grid type algorithms ensure that the components of a graph are placed on hierarchica layers within a
...
Regarding connection algorithms, most softwares today either use techniques such as shortest path connection
, orthogonal edge routing or various routing mechanics built around heuristics. Each approach tries to find a 
...
of finding the proper representation so that the number of intersetions is 0 is NP, therefore there is no
